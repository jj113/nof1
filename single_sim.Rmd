...
```{r eval=TRUE, include=TRUE, warning=FALSE, set.seed(2022)}
n = 1; w = 2
k = rep(kn, n)
...
run = myfun(power, alpha, method = "OBF", abs_delta = abs_delta, gamma = gamma, TT = TT[i,], class.save.in = class.save[i], k = k, n = n, n_pre = n, entry = entry[[i]], c_time = c_time_list[[i]], z_value = z_value, pstar = 1)
...
if(p[2] != 0){
  ...
  if(length(row.names(table(compare[wrong,])))>0){
     c0 = 0
